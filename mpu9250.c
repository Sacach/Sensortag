...
int mpu9250_get_data(I2C_Handle *i2c, float *ax, float *ay, float *az, float *gx, float *gy, float *gz) {
	...
	// JTKJ: 2. Muunnetaan 8-bittiset rekisterinarvot 16-bittisiksi. T舖s� siis 12
	...
	for(i=0; i < 14; i += 2){
		data[i/2] = (rawData[i] << 8) | rawData[i+1];
	}
	...
	*az = (float)data[2]*aRes - accelBias[2];
	...
	// JTKJ: 4. Muunnetaan g-arvot kulmanopeudeksi. T舂舅 muunnoksen voi tehd�
	//          halutesaan, kumpikin tapa datan esitykselle k艙
	...
	*gz = (float)data[6]*gRes;
	...
	// Datan tutkiminen
	// Viestin asetus
	if((*ax < -0.6) && (abs(*ay) < 0.35)){
		System_printf("vasen\n");
		System_flush();
		nykviesti = 1;
	}

	else if((*ax > 0.6) && (abs(*ay) < 0.35)){
		System_printf("oikea\n");
		System_flush();
		nykviesti = 2;
	}

	else if((abs(*ax) < 0.35) && (*ay < -0.6)){
		System_printf("ylos\n");
		System_flush();
		nykviesti = 3;
	}
